...
trait AutoReplayRepository {
  def setAutoReplayStatus(status: AutoReplayStatus): Future[Unit]
...
class AutoReplayRepositoryImpl @Inject()(
  ...
  def removeAll(): Future[Unit] = switchOn() // Test support FN

  def setAutoReplayStatus(status: AutoReplayStatus): Future[Unit] = status match {
      case Off => switchOff
      case _ => switchOn
    }
  ...
  private def switchOff(): Future[Unit] =
    Mdc.preservingMdc{
      collection
  ...
  private def switchOn(): Future[Unit] =
    Mdc.preservingMdc{
