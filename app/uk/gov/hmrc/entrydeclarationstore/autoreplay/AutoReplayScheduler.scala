...
class AutoReplayScheduler @Inject()(
  ...
  private def autoReplay(replaySequenceCount: Int = 1): Future[Unit] = {
    logger.info(s"Running AutoReplay sequence with replayCount = $replaySequenceCount")
    exclusiveTimePeriodLock
      ...
      .flatMap{
        ...
        case _ => Future.successful(logger.warn(s"AutoReplay sequence terminating on replayCount = $replaySequenceCount"))
      }
      .andThen {
