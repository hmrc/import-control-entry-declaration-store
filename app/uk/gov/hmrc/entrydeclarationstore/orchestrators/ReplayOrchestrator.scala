      ...
      .mapAsync(parallelism = 1) {
        case Right(counts) =>
          replayStateRepo
            ...
            .map((_, counts))
        case Left(Abort(err, counts)) if counts.successCount > 0 =>
        ...
        case Left(Abort(err, _)) =>
          logger.error(s"Batch replay failed, $err, throwing RuntimeException")
          throw new RuntimeException(s"Unable to replay batch $err")
      }
