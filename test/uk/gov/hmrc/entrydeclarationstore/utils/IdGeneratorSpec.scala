...
class IdGeneratorSpec extends AnyWordSpec with Matchers {
  ...
  val clientInfoWithCSP: ClientInfo = ClientInfo(ClientType.CSP, Some("beTrJdnR590oacIXerAZPaVpoIk3"), Some("wTne7a1e-1111-2222-3333-d77f17chpOsw"))
  ...
  "IdGenerator" when {
    ...
    "generating correlationIds" when {
      "generated ids for the same CSP user are different" in {
        ...
        ids.toSet.size shouldBe num
        ids.map(_.substring(10,14)).toSet mustBe Set("beTr")
      }
      ...
      "a NoSuchElementException exception is thrown in case CSP user has no clientId present" in  {
      ...
      "an IndexOutOfBoundsException is thrown in case CSP user's clientId is too short" in  {
        ...
        exception shouldBe a[IndexOutOfBoundsException]
      }
    }
