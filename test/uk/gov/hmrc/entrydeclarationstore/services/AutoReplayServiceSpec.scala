...
class AutoReplayServiceSpec
  ...
  "AutoReplayService" when {
    ...
    "setting autoReplay status" must {
      "set using the repo" in {
        val value = AutoReplayStatus.Off

        MockAutoReplayRepository.setAutoReplayStatus(value) returns Future.unit
        service.setStatus(value).futureValue
      }
    }
  }
}
