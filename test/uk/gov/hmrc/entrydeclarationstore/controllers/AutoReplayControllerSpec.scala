  ...
  "AutoReplayController" when {
    ...
    "setting autoReplay state" when {
      "setting true is successful" must {
        "return 204" in {
          MockAutoReplayService.setStatus(AutoReplayStatus.On) returns Future.unit
          val result = controller.setStatus()(FakeRequest().withBody(autoReplayStatusJson(true)))
      ...
      "setting false is successful" must {
        "return 204" in {
          MockAutoReplayService.setStatus(AutoReplayStatus.Off) returns Future.unit
          val result = controller.setStatus()(FakeRequest().withBody(autoReplayStatusJson(false)))
